            ...
            try:
                async for interface_id in self.__create_interface_datastore(data, {'interface': name}):
                    ...
                    if lag_proto in ('lacp', 'loadbalance'):
                        xmit = data['xmit_hash_policy'].lower() if data['xmit_hash_policy'] else None
                        if xmit is None:
                            xmit = 'layer2+3'

                        if lag_proto == 'lacp':
                            lacpdu_rate = data['lacpdu_rate'].lower() if data['lacpdu_rate'] else None
                            if lacpdu_rate is None:
                                lacpdu_rate = 'slow'
