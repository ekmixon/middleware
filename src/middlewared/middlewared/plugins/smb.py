...
from middlewared.service import (accepts, job, private, SharingService,
                                 SystemServiceService, ValidationErrors, filterable)
...
class SMBService(SystemServiceService):
    ...
    async def configure(self, job, create_paths=True):
        ...
        if ha_mode == SMBHAMODE.CLUSTERED:
            ...
            """ldap_enabled = (
                await self.middleware.call('directoryservices.get_conf', 'directoryservice.ldap')
            )['enable']"""
    ...
    async def get_smb_ha_mode(self):
        ...
        gl_enabled = (
            await self.middleware.call('service.query', [('service', '=', 'glusterd')], {'get': True})
        )['enable']
    ...
    async def do_update(self, data):
        ...
        set to the correct value during the process of joining an AD domain. `workgroup` and `netbiosname` should
        have different values.

        `enable_smb1` allows legacy SMB clients to connect to the server when enabled.

        `aapl_extensions` enables support for SMB2 protocol extensions for MacOS clients. This is not a requirement
        for MacOS support but is currently a requirement for time machine support.
        ...
        The group specified as the SMB `admin_group` will be automatically added as a foreign group member of
        S-1-5-32-544 (builtin\admins). This will afford the group all privileges granted to a local admin.
